...
@section('home')
@section('title')
{{ $course->course_name }} | Ez Buy & Learn Online Course
@endsection
        ...
        <div class="col-lg-8 mr-auto">
            <div class="breadcrumb-content">
                <ul class="generic-list-item generic-list-item-arrow d-flex flex-wrap align-items-center">
                    ...
                    <li><a href="{{url('category/' . $course->main_category_id . '/' . $course['category']['category_slug'])}}">{{ $course['category']['category_name'] }}</a></li>
                ...
                <div class="d-flex flex-wrap align-items-center pt-3">
                   ...
                   @else
                   <h6 class="ribbon ribbon-lg mr-2 bg-3 text-white">{{ round($discount) }}% OFF</h6>
                   @endif
                ...
                <p class="pt-2 pb-1">Created by <a href="{{ route('instructor.details',$course->instructor_id) }}" class="text-color hover-underline">{{ $course['user']['name'] }}</a>
                </p>
                @auth
                ...
                @else
                <button class="btn btn-danger lh-28 mr-2 mb-2 btn-lg" data-toggle="modal" data-target="#exampleModal">
                    <i class="la la-message mr-1"></i>Chat with {{$course['user']['name']}}
                </button>
                @endif
                ...
                <div class="bread-btn-box pt-3">
                    <button class="btn theme-btn theme-btn-sm theme-btn-transparent lh-28 mr-2 mb-2">
                        ...
                        <span id="{{ $course->id }}" onclick="addToWishList(this.id)" class="swapping-btn" title="Add to Wishlist">Wishlist</span>
                    </button>
                       ...
                       <div class="curriculum-header d-flex align-items-center justify-content-between pb-4">
                           ...
                           <div class="curriculum-duration fs-15">
                            ...
                            if($course->duration % 60 > 0){
                                $totalHours = intval($course->duration/60);
                            }
                            else{
                            ...
                            @endphp
                            <span class="curriculum-total__hours"><strong class="text-black font-weight-semi-bold">Total hours:</strong> {{  $totalHours }}</span>
                           </div>
                       </div>
                       @php
                        ...
                        <div class="media media-card">
                            <div class="instructor-img">
                                <a href="teacher-detail.html" class="media-img d-block">
                                    <img class="lazy" src="{{ (!empty($course->user->photo) && ($course->user->photo != 'photo')) ? url('upload/instructor_images/'.$course->user->photo) : url('upload/no_image.jpg')}}"  alt="Avatar image">
                                </a>
...
{{--                                 <p class="fs-14 text-center pb-4">30-Day Money-Back Guarantee</p>
                                <div class="preview-course-incentives">
                                    ...
                                    <ul class="generic-list-item pb-3">
                                        <li><i class="la la-play-circle-o mr-2 text-color"></i>{{$totalHours}} hours on-demand video</li>
                    ...
                    <div class="card card-item">
                        <div class="card-body">
                            ...
                            <ul class="generic-list-item generic-list-item-flash">
                                <li class="d-flex align-items-center justify-content-between"><span><i class="la la-clock mr-2 text-color"></i>Duration</span> {{ $totalHours }} hours</li>
                                ...
                                @endphp
                                <li class="d-flex align-items-center justify-content-between"><span><i class="la la-lightbulb mr-2 text-color"></i>Skill level</span> {{ $level }}</li>
                                <li class="d-flex align-items-center justify-content-between"><span><i class="la la-users mr-2 text-color"></i>Students</span> {{ number_format($enrollmentCount) }}</li>
                                
                                {{-- <li class="d-flex align-items-center justify-content-between"><span><i class="la la-certificate mr-2 text-color"></i>Certificate</span> @if($course->certificate == 1)Yes @else No @endif</li> --}}
                            </ul>
                        </div>
                    </div><!-- end card -->
                    <div class="card card-item">
                    ...
                    <div class="card card-item">
                        ...
                        @endif
                            ...
                            <div class="view-all-course-btn-box">
                                <a href="{{route('show.all.category')}}" class="btn theme-btn w-100">View All Courses <i class="la la-arrow-right icon ml-1"></i></a>
                            </div>
                        </div>
                    </div><!-- end card -->
                </div><!-- end sidebar -->
...
<section class="related-course-area bg-gray pt-60px pb-60px">
    ...
    <div class="card card-item card-preview"  data-toggle="tooltip" data-tooltip-content="#tooltip_{{$inscourse->id}}">
    <div class="card-image" >
    ...
    <div class="card-body">
        ...
        @endphp
        <h6 class="ribbon ribbon-blue-bg fs-14 mb-3">{{ $level}}</h6>
        ...
        @endif
        <div id="{{ $inscourse->id }}" onclick="addToWishList(this.id)" class="icon-element icon-element-sm shadow-sm cursor-pointer" title="Add to Wishlist"><i class="la la-heart-o"></i></div>
        ...
        <div class="row align-items-center">
            ...
            <div class="col-lg-3">
                <div class="cta-btn-box text-right">
                    <a href="{{route('become.instructor')}}" class="btn theme-btn">Teach on Mydemy <i class="la la-arrow-right icon ml-1"></i> </a>
                </div>
            </div><!-- end col-lg-3 -->
        </div><!-- end row -->
    </div><!-- end container -->
...
<div class="tooltip_templates">
    <div id="tooltip_{{ $item->id }}">
        <div class="card card-item">
            <div class="card-body">
                <p class="card-text pb-2">By <a href="{{ route('instructor.details',$item->instructor_id) }}">{{ $item['user']['name'] }}</a></p>
                <h5 class="card-title pb-1"><a href="{{ url('course/details/'.$item->id.'/'.$item->course_name_slug) }}" class="d-block"> {{ $item->course_name }}</a></h5>
                ...
                <ul class="generic-list-item generic-list-item-bullet generic-list-item--bullet d-flex align-items-center fs-14">
                    ...
                    if($item->duration % 60 > 0){
                        $totalHours = intval($item->duration/60);
                    }
                    else{
    ...
    @endphp
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div id="{{ $item->id }}" onclick="addToWishList(this.id)" class="icon-element icon-element-sm shadow-sm cursor-pointer" title="Add to Wishlist"><i class="la la-heart-o"></i></div>
                </div>
            </div>
        </div><!-- end card -->
    </div>
...
<!-- Modal -->
<div class="modal fade modal-container" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="reportModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header border-bottom-gray">
                <div class="pr-2">
                    <h5 class="modal-title fs-19 font-weight-semi-bold lh-24" id="reportModalTitle">Chat with {{$course['user']['name']}}</h5> 
                </div>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true" class="la la-times"></span>
                </button>
            ...
            <div class="modal-body">
                <div class="input-box">
                    <div class="form-group">
                        <textarea class="form-control form--control pl-3" id="messageTextarea" placeholder="Message for {{$course['user']['name']}}" rows="5"></textarea>
                    </div>
                </div>
                <div class="btn-box text-right pt-2">
                    <button type="button" class="btn font-weight-medium mr-3" data-dismiss="modal">Cancel</button>
                    
                    <button type="button" class="btn theme-btn theme-btn-sm lh-30" id="sendMessageButton" data-receiver-id="{{ $course->instructor_id }}">Submit <i class="la la-arrow-right icon ml-1"></i></button>
